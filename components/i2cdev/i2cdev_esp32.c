                    ...
                    uint16_t len, uint8_t *data)
{
    ...
    if (!err) {
        ...
        ESP_LOGW("[port:%d, slave:0x%X] Read_ %d bytes from register 0x%X, data: %s", port, devAddr, length, regAddr, str);
    }
    ...
    if (err) {
#endif
        ESP_LOGE("[port:%d, slave:0x%X] Failed to read %d bytes from register 0x%X, error: 0x%X",
                     ...
                     uint16_t len, uint8_t *data)
{
    ...
    if (!err) {
        ...
        ESP_LOGW("[port:%d, slave:0x%X] Write %d bytes to register 0x%X, data: %s",
    ...
    if (err) {
#endif
        ESP_LOGE("[port:%d, slave:0x%X] Failed to write %d bytes to__ register 0x%X, error: 0x%X",
                    port, devAddr, length, regAddr, err);
    }

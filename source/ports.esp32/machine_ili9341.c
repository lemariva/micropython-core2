...
STATIC void machine_hw_ili9341_internal(machine_hw_ili9341_obj_t *self) {
    ...
    spi_master_init(self->display, self->cs, self->dc, -1, -1);

    mp_printf(&mp_plat_print, "SPI connection [OK].\n");

    lcdInit(self->display, self->model, self->width, self->height, self->offsetx, self->offsety);
...
mp_obj_t machine_hw_ili9341_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    // Parse args
	enum { ARG_dc, ARG_cs, ARG_width, ARG_height, ARG_offsetX, ARG_offsetY};
    ...
    if (self->base.type == NULL) {
        ...
        self->model = 0x9341;
        self->offsetx = args[ARG_offsetX].u_int;
        self->offsety = args[ARG_offsetY].u_int;

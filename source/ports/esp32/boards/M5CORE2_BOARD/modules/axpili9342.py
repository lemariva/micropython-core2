###############################################################################
# Pure/Hybrid micropython lvgl display driver for ili9342 connected with AXP192
#
# For ili9342 display:
#
...
class ili9XXX:
    ...
    def __init__(
        ...
        self.buf2 = (
            esp.heap_caps_malloc(self.buf_size, esp.MALLOC_CAP.DMA) if double_buffer else None
        )
        ...
        self.disp_drv.flush_cb = (
            esp.ili9xxx_flush if hybrid and hasattr(esp, "ili9xxx_flush") else self.flush
        )
    ...
    def disp_spi_init(self):
        ...
        def post_isr(arg):
            ...
            if reported_transmitted > 0:
                print(
                    "- Completed DMA of %d bytes (mem_free=0x%X)"
...
class ili9342(ili9XXX):
    def __init__(
        ...
        if lv.color_t.SIZE != 2:
            raise RuntimeError("ili9342 micropython driver requires defining LV_COLOR_DEPTH=16")
        if colormode == COLOR_MODE_BGR and not hasattr(lv.color_t().ch, "green_l"):
            raise RuntimeError("ili9342 BGR color mode requires defining LV_COLOR_16_SWAP=1")
